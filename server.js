...
CHANNELS = {
	...
	tasty:{title:"tasty network music", fetch:[{type:"user", user:"TastyNetwork", orderby:"viewCount", count:9999}]},
	//dubs:{title:"dubstep by tasty", fetch:[{type:"user", user:"TastyDubstepNetwork", orderby:"viewCount", count:9999}]},
	music:{title:"some decent", fetch:[
	...
	blackhole:{title:"good trance and whatnot", fetch:[{type:"user", user:"BlackHoleRecordings", orderby:"viewCount", count:250}]},
	sneaky:{title:"Sneaky Sound System", fetch:[{type:"query", q:"sneaky sound system"}]},
	whoseline:{title:"improv ftw", fetch:[{type:"query", q:"whose line is it anyway", count:400}], fix_order:true},
	alton2:{title:"eats that are good", fetch:[{type:"query", q:"good eats -LikeTheHat", count:250}], fix_order:true},
	...
	cooking:{title:"ONLINE COOKING SHOW", fetch:[
...
HTTP_STATUSES = {302:"Found", 304:"Not Modified", 400:"Bad Request", 404:"Not Found", 500:"Internal Server Error", 501:"Not Implemented"};
...
Room.prototype.sync = function(user) {
	...
	user.send("sync", {users:user_list, video:this.video, video_time:video_time,
...
routes['hello'] = function(client, data) {
	...
	user.send("welcome", {user:user.pack(),
	...
	for(var i in EMOTICONS) {
		...
		while(true) {
			...
			if(new_msg_html != msg_html) {
				...
				emoticons.push({id:emoticon_id, frames:e.frames, width:e.width, height:e.height, step_ms:e.step_ms, image:i+".png"});
				emoticon_id++;
...
function socket_io_handler(client) {
	// new client is here!
	client.on('message', function(msg) { socket_io_on_message(client, msg); });
	client.on('disconnect', function() { socket_io_on_disconnect(client); });
}
