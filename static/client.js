...
function build_user_list() {
	...
	$(".user_dropdown_clickable").hover(function(){}, function() {
...
function handle_response(resp) {
	...
	// messages
		...
		for(var i in data.emoticons) {
			...
			$("#emoticon_"+e.id).css({width:e.width, height:e.height, background:"url("+e.image+") 0px 0px no-repeat"});
	...
	$("#what_is").click(function() {
		...
		if(arrow.hasClass("what_is_arrow_right")) {
			...
			arrow
				.removeClass("what_is_arrow_down")
				.addClass("what_is_arrow_right");
			$("#what_is_text").stop(true).animate({height:0}, {duration:'fast', complete:function(){$(this).hide()}});
		}
		return false;
	});
